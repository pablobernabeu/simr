...
powerCurve <- function(
    ...
    if(substr(test, 1, 5) == 'fixed') {
        if(gsub("^fixed\\(.(.+).\\)$", "\\1", test) %in% unique(names(fixef(fit))) == FALSE) stop("Please double-check the name of the effect in the model, especially the order of the variables in interactions.")
    }
